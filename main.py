...
class CSVDataFrame:
    ...
    def calculate_mean_login_logout(self, date_range: Tuple[datetime, datetime] = None):
        if not date_range:
            ...
            self.logout_mean = pd.to_datetime(self.timesheet['Logout'].dropna(), format='%H.%M').mean().strftime(
                '%H:%M')
...
class Timesheet:
    ...
    def get_num_workdays_in_month(year: int = datetime.now().year, month: int = datetime.now().month - 1) -> int:
        ...
        holidays = pd.to_datetime(
            ['2023-01-01', '2023-01-06', '2023-02-14', '2023-02-19', '2023-03-26', '2023-04-02', '2023-04-06',
             '2023-04-07', '2023-04-09', '2023-04-10', '2023-05-05', '2023-05-14', '2023-05-18', '2023-05-28',
             '2023-05-29', '2023-06-05', '2023-06-23', '2023-06-24', '2023-10-29', '2023-10-31', '2023-11-11',
             '2023-12-24', '2023-12-25', '2023-12-26', '2023-12-31', '2024-01-01', '2024-01-06', '2024-02-11',
             '2024-02-14', '2024-03-24', '2024-03-28', '2024-03-29', '2024-03-31', '2024-04-01', '2024-04-26',
             '2024-05-09', '2024-05-12', '2024-05-19', '2024-05-20', '2024-06-05', '2024-06-23', '2024-06-24',
             '2024-10-27', '2024-10-31', '2024-11-11', '2024-12-24', '2024-12-25', '2024-12-26', '2024-12-31'])
    ...
    def plot_surplus_deficit(self):
        ...
        hours.total['Surplus/Deficit'] = hours.total.apply(
            lambda row: (pd.to_datetime(row['Logout'], format='%H.%M') - pd.to_datetime(row['Login'],
                                                                                        format='%H.%M')).total_seconds() / 3600 - 7.4,
            axis=1)
